...
const NurseMessaging = () => {
  ...
  useEffect(() => {
    const channel = supabase
      .channel('db_session_states_nurse')
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', background: '#F8FAFC', padding: '6px 12px', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
                  ...
                  <span style={{ fontSize: '12px', fontWeight: 700, color: (activeH !== 'twin' || selectedPatient?.session_state?.is_emergency) ? (activeH === 'doctor' ? '#ef4444' : '#d97706') : '#94a3b8' }}>
                    HUMAN OVERRIDE
                  </span>
                </div>
              </div>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <p style={{ color: '#475569', fontSize: '14px', lineHeight: 1.5, marginBottom: '24px' }}>
              This patient is in an alert zone. Do you want to take over this chat manually as a Nurse?
            </p>
                ...
                onClick={async () => {
                  ...
                  try {
                    await fetch('http://127.0.0.1:8000/handover', {
                      method: 'POST',
                      headers: { 'Content-Type': 'application/json' },
                      body: JSON.stringify({ user_id: pId, target_handler: 'nurse' })
                    });
                    setPatients(prev => prev.map(pt => pt.id === pId ? {...pt, session_state: {...pt.session_state, active_handler: 'nurse', is_emergency: true}} : pt));
                    setSelectedId(pId);
                    setHumanMessage('');
                  } catch (e) {
                    console.error(e);
